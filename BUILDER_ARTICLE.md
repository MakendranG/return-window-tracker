...
Copy each field below into the matching box in the "Create an article" editor at
https://builder.aws.com. The BODY section (after the divider) goes in the Body field.
-->

# ── FIELDS (copy into the form) ─────────────────────────────────────────────
...
## Cover image suggestion  (optional, 1200x675)
A simple flow graphic: "Purchases → Strands Agent (deterministic tools) → LLM drafting → Digest". Avoid text-heavy images per AWS guidance.

---

# ── BODY (paste everything below into the Body field) ───────────────────────
...
2. **`decide_autonomy`** — pure Python policy. It decides, by auditable rules, whether the agent can handle an item on its own (`AUTO_HANDLED`) or must surface it to the human (`NEEDS_YOU`). It escalates when the item is expensive (≥ $150), the deadline is today/tomorrow, the user's reason sounds uncertain, or the window already closed on something they wanted to return.

Here's the tool that encodes the "only surface real decisions" behavior:
...
def decide_autonomy(items: list[dict]) -> dict:
    ...
    for it in items:
        ...
        if any(m in (it.get("reason_for_potential_return") or "").lower()
               for m in ambiguity_markers):
            triggers.append("your reason sounds uncertain")
        # ...NEEDS_YOU when any trigger fires, else AUTO_HANDLED
```

The LLM then reads the tool outputs and does what it's genuinely good at: drafting the return-request messages and writing the human-readable daily digest — always reusing the tools' exact numbers.

## Where AWS comes in

**Amazon Bedrock** is the foundation-model runtime for the whole thing. The Strands Agents SDK defaults to Bedrock (Claude Sonnet), so the agent's reasoning and the natural-language drafting all run on Bedrock. Concretely, AWS does the work in two places:
...
The same `agent.py` core powers three interfaces: a CLI, a Streamlit web UI, and the AgentCore entrypoint — no logic duplicated.

## Making it reliable: structured output over hand-written JSON

My first version asked the model to emit JSON directly. It worked… until the model wrote `"I\'ll"` inside a string and the JSON parser choked. Rather than patch escaping bugs forever, I switched to **Strands structured output** with a Pydantic schema:
...
This means a judge can click the link and immediately see the agent working, then optionally flip to live Bedrock on their own account.
